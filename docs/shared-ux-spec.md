...
| Translation succeeds with sound off | Nothing is spoken and every replay control is disabled; the bubble is unchanged |
...
| Run the app on a phone set to French or Spanish | The interface is in that language, including the status strip, the error banners, and the first-run flow |
